// ============================================================
//  CreateQuestion.js
//  Handles paper question management: view, edit, approve,
//  reorder, comment, and CLO assignment.
// ============================================================
...
// ── Local Components ──────────────────────────────────────────
import LabAssignQuestionModal from "./LabAssignQuestionModal";
import LabReorderQuestionsModal from "./LabReorderQuestionsModal";
import LabCheckPolicyModal from "./LabCheckPolicyModal";
...
//  Shows diff between original and edited question text.
//  - Yellow highlight  → added text
...
const HighlightedText = ({ original = "", edited = "" }) => {
  ...
  // No change — just render plain text
    ...
    <View style={styles.diffBox}>
      <Text style={styles.normalText}>
        {diffs.map((part, index) => {
          ...
          if (!text) return null;
          if (type === 0) return <Text key={index}>{text}</Text>;                                          // Unchanged
          if (type === 1) return <Text key={index} style={styles.addedText}>{text}</Text>;                 // Added
          if (type === -1) return <Text key={index} style={styles.removedText}>{text}</Text>;              // Removed
...
// ============================================================
//  MAIN COMPONENT: CreateQuestion
// ============================================================
const LabCreateQuestion = () => {
  ...
  // ── Import Report (unused in UI but kept for future) ────────
  ...
  // Filtered CLO list based on search input
  ...
  // Lock editing if question is already approved or rejected
  const isQuestionLocked =
    selectedQuestion?.Status === "approved" ||
    selectedQuestion?.Status === "rejected";

  // Whether user can edit this question
  ...
  // Whether user is adding an extra (bonus) question
  const isExtraTab = editingIndex === "extra";

  // Safe string helper to avoid rendering errors
  const safeText = (val) =>
    typeof val === "string" || typeof val === "number" ? String(val) : "";

  // Image URI for edit form (new preview OR existing server image)
  ...
  // Fetch paper details on mount / paperId change
  useEffect(() => {
  ...
  // Fetch CLOs whenever courseId changes
  useEffect(() => {
    if (!courseId) return;
  ...
  // Fetch paper comments when modal opens
  useEffect(() => {
  ...
  // Verify teacher-course relationship and set permissions
  ...
  // Begin editing an existing question
  ...
  // Switch to "extra question" creation mode
  ...
  // Submit edited/new question to backend
  const handleEditSubmit = async () => {
    try {
      ...
      // Extra question needs paper_id and isextra flag
      ...
      if (editForm.image) {
        formDataToSend.append("image", {
          uri: Platform.OS === "android"
            ? editForm.image.uri
            : editForm.image.uri.replace("file://", ""),
  ...
  // Delete an extra question after confirmation
  ...
  // Request storage permission (Android legacy — currently returns true)
  ...
  // Open image picker and update edit form
  ...
  // Remove image from edit form and flag removal
  const removeImage = () => {
    setEditForm((prev) => ({
      ...prev,
      image: null,
  ...
  // ============================================================
  //  PAPER WORKFLOW ACTIONS
  // ============================================================

  // Reorder questions and persist to backend
  ...
  // Send paper to Faculty Approver
        ...
        onPress: async () => {
          try {
            const response = await axios.post(
              `${BASE_URL}/paper/sendToFacultyApprover/${paperDetails.PaperId}`
            );
            setPaperDetails((prev) => ({
              ...prev,
              PaperStatus: response.data?.PaperStatus || "getFacultyApprover",
            }));
  ...
  // Director approves the whole paper
          ...
          try {
            ...
            setPaperDetails((prev) => {
              const updatedQuestions = prev.Questions.map((q) =>
                q.Id === questionId ? { ...q, Status: status } : q
              );
              return { ...prev, Questions: updatedQuestions };
            });
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        >
          ...
          <View style={styles.paperHeader}>
            ...
            {/* Paper Status Badge / Action Button */}
            <TouchableOpacity
              style={[
                styles.statusBadge,
          ...
          >
            {questions.map((q, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.tabBtn,
                  activeTab === index ? styles.tabActive : null,
                  q.IsExtra ? styles.extraQuestionTab : null,
                ]}
                onPress={() => setActiveTab(index)}
              >
                <Text style={styles.tabText}>
                  {q.IsExtra ? `Q${index + 1} (Extra)` : `Q${index + 1}`}
                </Text>
              </TouchableOpacity>
            ))}

            {/* Add Extra Question Tab */}
            {createPaper && (
              <TouchableOpacity
                style={[styles.tabBtn, isExtraTab ? styles.tabActive : null]}
          ...
          <View style={styles.questionsGrid}>
            {(selectedQuestion || isExtraTab) && (
              <View style={[styles.questionCard, editingIndex !== null ? styles.editingCard : null]}>
                ...
                {/* Card Header: Q number + action buttons */}
                <View style={styles.questionCardHeader}>
                  <Text style={styles.questionNumber}>
                    {isExtraTab
                      ? `Extra Q${questions.length + 1}`
                      : `Q${activeTab + 1}${selectedQuestion?.IsExtra ? " (Extra)" : ""}`}
                  </Text>
                  ...
                  <View style={styles.leftActions}>
                    ...
                    {!isExtraTab && editingIndex !== activeTab && canEdit && (
                      <TouchableOpacity
                        style={[styles.editButton, isQuestionLocked && { opacity: 0.5 }]}
                    ...
                    {!isExtraTab && editingIndex !== activeTab && canEdit && createPaper && (
                      <TouchableOpacity
                        style={styles.reorderBtn}
                        onPress={() => setShowAssignQuestionModal(true)}
                      >
                    ...
                    {/* Delete Extra Question Button */}
                    {(createPaper || isUserDirector) &&
                      selectedQuestion?.IsExtra &&
                      !isExtraTab &&
                      editingIndex == null && (
                        <TouchableOpacity
                ...
                {editingIndex !== null && (editingIndex === activeTab || isExtraTab) ? (
                        ...
                        {["easy", "medium", "tough"].map((level) => (
                          <TouchableOpacity
                            ...
                            style={[
                              ...
                              editForm.difficulty === level ? styles.difficultyActive : null,
                            ]}
                            onPress={() => setEditForm((prev) => ({ ...prev, difficulty: level }))}
                          >
                  ...
                  !isExtraTab && selectedQuestion && (
                    <View style={styles.questionContent}>
                      ...
                      {/* Show diff if edited, otherwise plain text */}
                      ...
                      {/* Question Tags + Approve/Reject */}
                      <View style={styles.questionTags}>
                        <Text style={[styles.tag, styles.marksTag]}>
        ...
        <LabAssignQuestionModal
          visible={showAssignQuestionModal}
          paperId={paperDetails.PaperId}
          selectedQuestionId={selectedQuestion?.Id}
          courseId={courseId}
          onClose={() => setShowAssignQuestionModal(false)}
        />
          ...
          >
            <View style={styles.commentsModal}>
              <View style={styles.commentsHeader}>
                ...
                <TouchableOpacity onPress={() => setShowQuestionComments(false)}>
                  <Text style={styles.modalClose}>✕</Text>
                </TouchableOpacity>
              </View>

              <ScrollView
                ref={questionCommentsScrollRef}
                style={styles.commentsBody}
                contentContainerStyle={{ paddingBottom: 16 }}
              >
                  ...
                  questionComments.map((c, i) => {
                    ...
                    return (
                      <View key={i} style={[styles.commentItem, isSender ? styles.sent : styles.received]}>
                        ...
                        <Text style={styles.commentDate}>
                          {c.CommentDate ? new Date(c.CommentDate).toLocaleString() : "Unknown"}
                        </Text>
                      </View>
                    );
                  })
              ...
              <View style={styles.commentsFooter}>
                <TextInput
                  ...
                  onChangeText={setNewQuestionComment}
                  placeholder="Write a comment..."
                />
                <TouchableOpacity
                  style={styles.sendBtn}
                  onPress={() => handleAddQuestionComment(selectedQuestion?.Id)}
                >
                  <Text style={styles.sendBtnText}>Send</Text>
                </TouchableOpacity>
              </View>
        ...
        <LabCheckPolicyModal
          ...
          onPolicyCheck={(status) => setPolicyStatus(status)}
        />
...
export default LabCreateQuestion;
...
const styles = StyleSheet.create({
  ...
  reorderTextBtn: { color: "#fff", fontWeight: "500" },
  commentBtn: {
    paddingVertical: 8,
    paddingHorizontal: 12,
  ...
  commentText: { color: "#fff", fontWeight: "500", marginLeft: 4 },
  deleteButton: {
    paddingVertical: 8,
    paddingHorizontal: 12,
  ...
  previewImg: {
    width: "100%",
    height: Math.round(SCREEN_WIDTH * 0.45),
    resizeMode: "cover",
  },
  removeImageBtn: {
    position: "absolute",
  ...
  normalText: { fontSize: 16, color: "#374151" },
  addedText: {
    backgroundColor: "#dbe11b",
    color: "#000",
    paddingHorizontal: 3,
    borderRadius: 4,
  },
  ...
  boldText: { fontWeight: "700", color: "#16a34a" },
  highlightedText: {
