...
const CurrentPapers = () => {
  ...
  const [activeTab, setActiveTab] = useState("All");
  const [termFilter, setTermFilter] = useState("All"); // ✅ added
  const [papers, setPapers] = useState([]);
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  ...
  const filteredPapers = useMemo(() => {
    ...
    if (termFilter !== "All") {
      filtered = filtered.filter(
        (p) => p.Term?.toLowerCase() === termFilter.toLowerCase()
      );
    }
    ...
    return filtered;
  }, [activeTab, termFilter, papers, searchQuery]);
  ...
  placeholderTextColor="#888"   // 👈 yahan set hota hai
  value={searchQuery}
  onChangeText={setSearchQuery}
/>
      {/* ✅ TERM FILTER (NEW) */}
      <View style={styles.termRow}>
        {["All", "mid", "final"].map((t) => (
          <TouchableOpacity
            ...
            style={[
              styles.termBtn,
              termFilter === t && styles.activeTerm,
            ]}
            onPress={() => setTermFilter(t)}
          >
            <Text style={{ color: termFilter === t ? "#fff" : "#000" }}>
              {t.toUpperCase()}
            </Text>
          </TouchableOpacity>
        ))}
