...
import RNPrint from "react-native-print"; // ✅ CHANGED THIS LINE ONLY
...
const MeetingScheduleByTimeTable = () => {
  ...
  const downloadPDF = async () => {
    try {
      ...
      await RNPrint.print({ html }); // ✅ Opens print dialog → user saves as PDF

      Alert.alert("Success", "Print dialog opened — save as PDF");
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const res = await axios.delete(`${BASE_URL}/TeacherFreeSlots/clear`);
              Alert.alert("Success", res.data?.message || "Timetable deleted successfully");
              fetchSlots();
            } catch (err) {
              console.log(err);
  ...
  const handleGenerate = async () => {
    try {
      setGenerating(true);
      const payload = { startDate, endDate, startTime, endTime, slotDuration, senderId: 1 };
      const res = await axios.post(`${BASE_URL}/Meetings/GenerateSchedule`, payload);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.cardBox}>
        ...
        {file && <Text style={{ marginTop: 5 }}>Selected: {file.name}</Text>}
      </View>

      <TouchableOpacity style={styles.btnRed} onPress={deleteTimetable}>
        <Text style={{ color: "#fff", fontWeight: "bold" }}>Delete All Timetable</Text>
      </TouchableOpacity>
      ...
      <View style={styles.cardBox}>
        ...
        <TouchableOpacity style={styles.input} onPress={() => openPicker("date", "startDate")}>
          <Text>{startDate || "Select Start Date"}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.input} onPress={() => openPicker("date", "endDate")}>
          <Text>{endDate || "Select End Date"}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.input} onPress={() => openPicker("time", "startTime")}>
          <Text>{startTime || "Select Start Time"}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.input} onPress={() => openPicker("time", "endTime")}>
      ...
      <Modal visible={showIssues} transparent animationType="slide">
        <View style={styles.modal}>
          <View style={styles.modalBox}>
            ...
            {issues.map((i, idx) => (
              <Text key={idx}>{i.TeacherName} - {i.Issue}</Text>
            ))}
            <TouchableOpacity style={styles.btn} onPress={() => setShowIssues(false)}>
...
const styles = {
  ...
  modalBox: { backgroundColor: "#fff", padding: 15, borderRadius: 12 },
  btnRed: { backgroundColor: "#d32f2f", padding: 12, borderRadius: 8, alignItems: "center", marginVertical: 10 },
};